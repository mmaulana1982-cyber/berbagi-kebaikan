...
export default function App() {
  ...
  // Live Automatic Synchronization with Google Apps Script & Google Sheets
  useEffect(() => {
    if (!settings.isGasSyncEnabled || !settings.gasWebhookUrl) return;
    ...
    const syncLiveData = async () => {
      ...
      try {
        const res = await storageService.fetchLiveFromGas();
        ...
        console.warn('[LiveSync] Background fetch notice:', e);
      }
    };

    // Initial fetch on mount / settings update
    syncLiveData();

    // Live polling interval (every 25 seconds)
    const interval = setInterval(syncLiveData, 25000);

    // Sync on tab visibility change / window focus
  ...
  }, [settings.isGasSyncEnabled, settings.gasWebhookUrl]);
