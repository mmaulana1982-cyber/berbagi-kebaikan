import { AppSettings, Campaign, Disbursement, Donation, DonorPrayer, Comment, Announcement } from '../types';
...
const STORAGE_KEYS = {
  ...
  COMMENTS: 'sj_comments_v1',
  ANNOUNCEMENTS: 'sj_announcements_v1',
...
export const storageService = {
  ...
  addComment(comment: Comment): void {
    ...
    this.saveComments([comment, ...comments]);
  },

  deleteComment(id: string): void {
    const updated = this.getComments().filter(c => c.id !== id);
  ...
  addAnnouncement(announcement: Announcement): void {
    ...
    this.saveAnnouncements([announcement, ...announcements]);
  },

  deleteAnnouncement(id: string): void {
    const updated = this.getAnnouncements().filter(a => a.id !== id);
